...
addpath('C:/cliam-b/Desktop/Grad School/Y1/Data Visualization/Module 1/temperature-data-lab-part-1-kilo-s-fan-club/ObsData')
...
for i=1:length(sta)
    ...
    [P_all(i,:), P_recent(i,:)] = StationTempObs_LinearTrend(station_number,RecentYear)
end
...
Smooth = NaN(length(sta), length(stationdata.Year))
...
for i = 1:length(sta)
    station_number = sta(1,i)
    [baseline_model(i,:), P(i,:), Anomaly(i,:), Smooth(i,:)] = StationModelProjections(station_number)
end 
...
for i = 1:length(sta)
   ...
   for j = 2006:2099
        ...
        if (temperature_change >= (baseline_model(i,2)* 2))
...
load coastlines
plotm(coastlat,coastlon)
scatterm(lat,lon, 90, Emergence_year(:,1), "filled") % reading the std dev (2nd column) of the linear trend line from our function
colorbar
title('Year of long-term temperature signal')
...
plot(stationdata.Year, Smooth(i,:), 'DisplayName', string(sta(1,i))) 
legend
hold on 
end
